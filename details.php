...
$blog_id = !empty($_GET['blog_id']) ? (int)$_GET['blog_id'] : 0;
...
// specify template
$GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_details.tpl';
...
$path = $weblogcat->getNicePathFromId($entryObject->getVar('cat_id'), sprintf('%s/modules/%s/index.php?', XOOPS_URL, $xoopsModule->dirname()));
...
$xoopsTpl->assign('created_date', formatTimestamp($entryObject->getVar('created'), $xoopsModuleConfig['dateformat'], $xoopsConfig['default_TZ']));
$xoopsTpl->assign('created_time', formatTimestamp($entryObject->getVar('created'), $xoopsModuleConfig['timeformat'], $xoopsConfig['default_TZ']));
...
$xoopsTpl->assign('read_users_blog', sprintf('<a href="index.php?user_id=%d">%s</a>', $entryObject->getVar('user_id'), sprintf(_BL_READ_USERS_BLOG, $entryObject->getVar('uname'))));
...
$xoopsTpl->assign('provide_edit_link', ($isAdmin
                                        || ($currentuid == $entryObject->getVar('user_id')
                                            && checkprivilege('edit', $xoopsModule->dirname()))) ? 1 : 0);
