...
// POST variables
$lid = empty($_POST['lid']) ? 0 : (int)$_POST['lid'];
...
if (count($cats) > 0) {
    ...
    foreach ($cats as $cat) {
        ...
        if ($cid == $cat['cat_id']) {
            $cat_options .= "<option value='{$cat['cat_id']}' selected>$prefix{$newcat} ($photo_count)</option>\n";
        } else {
            $cat_options .= "<option value='{$cat['cat_id']}'>$prefix{$newcat} ($photo_count)</option>\n";
        }
    }
