...
if (!class_exists('WeblogEntryBase')) {
    class WeblogEntryBase extends XoopsObject
    {
        ...
        public function &getVar($key, $format = 's', $blog_id = 0, $contents_mode = false)
        {
            ...
            $ret      = $this->parse_viewmode($contents, $blog_id, $contents_mode, $this->moduleDirName);

            return $ret;
        }

        // parse entry->content in case of using separator functions
        public function parse_viewmode($contents, $blog_id, $contents_mode = 'details', $moduleDirName = '')
        {
            global $xoopsUser, $xoopsModuleConfig, $xoopsModule, $xoopsConfig;

            $moduleDirName = empty($moduleDirName) ? $this->moduleDirName : $moduleDirName;
            ...
            if (!isset($xoopsModuleConfig['minentrysize'])) {    // this config is sapmle.
                ...
                $module                = $module_h->getByDirname($moduleDirName);
            ...
            switch ($contents_mode) {
                ...
                case 'details':    // parse member limit and erase first-last half separator
                    ...
                    if ($currentuid) {
                        ...
                        $contents = preg_replace('/(' . MEMBER_ONLY_READ_DELIMETER . ").*$/sm", "<br><br><a href='" . XOOPS_URL . WEBLOG_REGISTER_LEADING_PAGE . "'>" . _BL_MEMBER_ONLY_READ_MORE . "</a><br>\n", $contents);
                    }
                ...
                case 'index':    // parse member limit and first-last half
                    ...
                    if (isset($xoopsModuleConfig['use_separator']) && $xoopsModuleConfig['use_separator']) {
                        $weblog_division_next_string = sprintf("<br><br><a href=\"%s/modules/%s/details.php?blog_id=%d\">%s</a><br>\n", XOOPS_URL, $moduleDirName, $blog_id, _BL_ENTRY_SEPARATOR_NEXT);
                        $contents                    = preg_replace('/(' . _BL_ENTRY_SEPARATOR_DELIMETER . ").*$/sm", $weblog_division_next_string, $contents);
                    } else {
                        $contents = str_replace(_BL_ENTRY_SEPARATOR_DELIMETER, '', $contents);
                    }
                    break;
    ...
    class WeblogEntryHandlerBase extends XoopsObjectHandler
    {
        ...
        public function delete(XoopsObject $entry)
        {
            ...
            $sql = sprintf('DELETE FROM %s WHERE blog_id=%d LIMIT 1', $this->db->prefix($this->moduleDirName), $entry->getVar('blog_id'));
            if (!$result = $this->db->query($sql)) {  // must be query()
        ...
        {
            $sql = sprintf('SELECT count(*) AS count FROM %s', $this->db->prefix($this->moduleDirName));
        ...
        {
            $blog_id = (int)$blog_id;
            $sql     = sprintf('UPDATE %s SET `reads` = `reads` + 1 WHERE blog_id=%d', $this->db->prefix($this->moduleDirName), $blog_id);
            if (!$result = $this->db->queryF($sql)) {  // must be queryF()

                return -1;
            }
            $sql = sprintf('SELECT `reads` FROM %s WHERE blog_id=%d', $this->db->prefix($this->moduleDirName), $blog_id);
            if (!$result = $this->db->query($sql)) {
                return -1;
            }
        ...
        public function incrementTrackbacks($blog_id, $increment_num = 1)
        {
            ...
            $sql = sprintf('UPDATE %s SET trackbacks = trackbacks %s WHERE blog_id=%d', $this->db->prefix($this->moduleDirName), $increment_num, $blog_id);
            if (!$result = $this->db->queryF($sql)) {  // must be queryF()
        ...
        public function updateComments($blog_id, $total_num)
        {
            ...
            $sql       = sprintf('UPDATE %s SET comments=%d WHERE blog_id=%d', $this->db->prefix($this->moduleDirName), $total_num, $blog_id);
            if (!$result = $this->db->queryF($sql)) {  // must be queryF()

                return -1;
            }

            $sql = sprintf('SELECT comments FROM %s WHERE blog_id=%d', $this->db->prefix($this->moduleDirName), $blog_id);
            if (!$result = $this->db->query($sql)) {
                return -1;
            }
        ...
        public function getPrevNextBlog_id($blog_id, $created, $criteria)
        {
            ...
            if ($created <= 0 && $blog_id < 0) {
                ...
                $rs      = $this->db->query(sprintf('SELECT created FROM %s WHERE blog_id=%d', $this->db->prefix($this->moduleDirName), $blog_id));
                $result  = $this->db->fetchArray($rs);
                $created = $result['created'];
            }

            $sql_prev = sprintf('SELECT blog_id FROM %s WHERE created<%d %s ORDER BY created DESC LIMIT 1', $this->db->prefix($this->moduleDirName), $created, $extra);
            $sql_next = sprintf('SELECT blog_id FROM %s WHERE created>%d %s ORDER BY created LIMIT 1', $this->db->prefix($this->moduleDirName), $created, $extra);
...
$entry_class = (string)(ucfirst($moduleDirName) . 'Entry');
if (!defined($entry_class) && isset($GLOBALS['moduleDirName'])) {
    define($entry_class, 'DEFINED CLASS');

    eval('
    class ' . ucfirst($GLOBALS['moduleDirName']) . 'Entry extends WeblogEntryBase{
...
');
    ...
    eval('
    class ' . ucfirst($GLOBALS['moduleDirName']) . 'EntryHandler extends WeblogEntryHandlerBase{
        var $moduleDirName ="' . $GLOBALS['moduleDirName'] . '" ;
        function &create() {
            $ret = new ' . ucfirst($GLOBALS['moduleDirName']) . 'Entry();
