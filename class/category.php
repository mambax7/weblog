...
if (!class_exists('WeblogCategoryBase')) {
    ...
    {
        public $moduleDirName = '';        // abstract
        public $mytree;

        public function create()
        {
            return new WeblogCategoryBase();    //abstract
        }
        ...
        public function delete(XoopsObject $category)
        {
            ...
            $sql = sprintf('DELETE FROM %s WHERE cat_id=%d LIMIT 1', $this->db->prefix($this->moduleDirName . '_category'), $category->getVar('cat_id'));
            if (!$result = $this->db->queryF($sql)) {  // must be queryF()
        ...
        {
            $sql = sprintf('SELECT count(*) AS count FROM %s', $this->db->prefix($this->moduleDirName . '_category'));
         ...
         * @param int    $cat_id
         * @param string $order
         * @return array created by hodaka
         * created by hodaka
         */
        ...
        public function getTreeInstance()
        {
            static $instance;
            if (null === $instance) {
                $instance = new wbXoopsTree($this->db->prefix($this->moduleDirName . '_category'), 'cat_id', 'cat_pid');
            }
    ...
    {
        //constructor
        public function __constructor($table_name, $id_name, $pid_name)
        {
        ...
        public function makeMySelBox($title, $order = '', $preset_id = 0, $none = 0, $sel_name = '', $onchange = '')
        {
            ...
            if ((is_object($xoopsUser) && 'xoopsuser' === get_class($xoopsUser) && $xoopsUser->isAdmin($modid))
                ...
                $sql = sprintf('SELECT %s, %s FROM %s, %s WHERE %s=0 ', $this->id, $title, $this->table, $this->db->prefix('group_permission'), $this->pid);
        ...
        public function getChildTreeArray($sel_id = 0, $order = '', $parray = [], $r_prefix = '')
        {
            ...
            if ((is_object($xoopsUser) && 'xoopsuser' === get_class($xoopsUser) && $xoopsUser->isAdmin($modid))
                ...
                $sql = sprintf('SELECT * FROM %s, %s WHERE %s=%d ', $this->table, $this->db->prefix('group_permission'), $this->pid, $sel_id, $modid);
                $sql .= $this->weblog_cat_gpermsql();
                $sql .= ' group by cat_id ';
            }
            if ('' != $order) {
...
$entry_class = (string)(ucfirst($moduleDirName) . 'Category');
if (!defined($entry_class)) {
    define($entry_class, 'DEFINED CLASS');

    eval('
    class ' . ucfirst($GLOBALS['moduleDirName']) . 'Category extends WeblogCategoryBase{
        public $moduleDirName ="' . $GLOBALS['moduleDirName'] . '" ;
...
');
    eval('
    class ' . ucfirst($GLOBALS['moduleDirName']) . 'CategoryHandler extends WeblogCategoryHandlerBase{
        public $moduleDirName ="' . $GLOBALS['moduleDirName'] . '" ;
        function &create() {
            $ret = new ' . ucfirst($GLOBALS['moduleDirName']) . 'Category();
