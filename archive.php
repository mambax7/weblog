...
$date    = isset($_GET['date']) ? (int)$_GET['date'] : 0;    // YYYYMM (index.php is unixtime)
$cat_id  = isset($_GET['cat_id']) ? (int)$_GET['cat_id'] : 0;
// $fromyear = isset($_GET['year']) ? (int)($_GET['year']): (($date > 0)? (int)(substr($date, 0, 4)) : 0);
// $frommonth = isset($_GET['month']) ? (int)($_GET['month']): (($date > 0 )? (int)(substr($date, 4, 2)) : 0);
// $fromday = isset($_GET['day']) ? (int)($_GET['day']): 0;
// $mode = isset($_GET['mode']) ? htmlspecialchars($_GET['mode']) : (($fromyear + $frommonth > 0)? "date" : "");
$start      = isset($_GET['start']) ? (int)$_GET['start'] : 0;
$numperpage = isset($_GET['numperpage']) ? (int)$_GET['numperpage'] : 30;
...
// specify template
$GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_archive.tpl';
...
while ($created_example = $xoopsDB->fetchArray($month_result)) {
    ...
    $months[$this_year . $this_month] = $this_year . '&nbsp;' . $months_arr[(int)$this_month];
}
