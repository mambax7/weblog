...
class Net_TrackBack
{
     ...
     * @var array
     */
    public $ping_keys = array('title', 'url', 'excerpt', 'blog_name');
     ...
     * @access public
     */
    public function sendPing($url, $data, $user_agent = NET_TRACKBACK_DEFAULT_USER_AGENT, $charset = null)
    {
        if (!is_array($data)) {
            return PEAR::raiseError('Invalid Ping Data', NET_TRACKBACK_ERROR_INVALID_PARAM);
        }
        ...
        if ($result == 0) {
            return PEAR::raiseError("Bad Response format from $url", NET_TRACKBACK_ERROR_BAD_RESPONSE);
        }
        $errnum = $matches[1];
        if ($errnum != 0) {
            ...
            return PEAR::raiseError('Server returns error: ' . $message, NET_TRACKBACK_ERROR_REMOTE_ERROR);
        }
    ...
    public function getId()
    {
        ...
        if (substr($pathinfo, 0, 1) == '/') {
            ...
            return PEAR::raiseError('Invalid PATH_INFO', NET_TRACKBACK_ERROR_INVALID_PATHINFO);
        }
        $pos = strpos($id, '/');
        if ($pos !== false) {
            $id = substr($id, 0, $pos);
    ...
    public function toRSSXML(
        ...
        foreach ($items as $item) {
            ...
            foreach ($keys as $key => $itemname) {
                if (isset($item[$key])) {
                    $part .= sprintf("<%s>%s</%s>\n", $itemname, htmlspecialchars($item[$key]), $itemname);
                }
            }
            if (!empty($part)) {
                $rss .= "<item>\n" . $part . "</item>\n";
    ...
    public function toEmbededRDF($data)
    {
        ...
        foreach ($keys as $key) {
            if (!array_key_exists($key, $data)) {
                PEAR::raiseError("$key is required key in data", NET_TRACKBACK_ERROR_INVALID_PARAM);
            }
        }

        return $rdfspec . sprintf($rdffmt, $data['about'], $data['ping'], $data['title'], $data['identifier'], $data['description'], $data['creator'], $data['date']);
    }
}
