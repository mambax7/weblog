...
function myalbum_updaterating($lid)
{
    ...
    $xoopsDB->query($query) || die('Error: DB update rating.');
}
    ...
    if (isset($valid)) {
        ...
        if ($valid == 1) {
            ...
            $notificationHandler->triggerEvent('global', 0, 'new_photo', [
                'PHOTO_TITLE' => $title,
                'PHOTO_URI'   => "$mod_url/photo.php?lid=$lid&cid=$cid"
            ]);
...
function myalbum_delete_photos($whr)
{
    ...
    while (list($lid, $ext) = $xoopsDB->fetchRow($prs)) {
        ...
        //      $xoopsDB->query( "DELETE FROM $table_votedata WHERE lid=$lid" ) || die( "DB error: DELETE votedata table." ) ;
        //      $xoopsDB->query( "DELETE FROM $table_text WHERE lid=$lid" ) || die( "DB error: DELETE text table." ) ;
        $xoopsDB->query("DELETE FROM $table_photos WHERE lid=$lid") || die('DB error: DELETE photo table.');
...
function myalbum_get_cat_options(
    $order = 'title',
    $preset = 0,
    $prefix = '--',
    $none = null,
    ...
    if (empty($table_name_cat)) {
        $table_name_cat = $GLOBALS['table_cat'];
    }
    if (empty($table_name_photos)) {
        $table_name_photos = $GLOBALS['table_photos'];
    }

    $cats[0] = ['cid' => 0, 'pid' => -1, 'next_key' => -1, 'depth' => 0, 'title' => '', 'num' => 0];
