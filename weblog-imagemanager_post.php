...
require_once XOOPS_ROOT_PATH . '/modules/' . $moduleDirName . '/include/myalbum_imagemanager/include/read_configs.php';
...
if (!empty($_POST['submit'])) {
    ...
    /*
        ...
        if ($status) {
            ...
            // Global Notification
            $notificationHandler->triggerEvent( 'global' , 0 , 'new_photo' , array( 'PHOTO_TITLE' => $title , 'PHOTO_URI' => "$mod_url/photo.php?lid=$newid&cid=$cid" ) ) ;

            // Category Notification
            $rs = $xoopsDB->query( "SELECT title FROM $table_cat WHERE cid=$cid" ) ;
            list( $cat_title ) = $xoopsDB->fetchRow( $rs ) ;
            $notificationHandler->triggerEvent( 'category' , $cid , 'new_photo' , array( 'PHOTO_TITLE' => $title , 'CATEGORY_TITLE' => $cat_title , 'PHOTO_URI' => "$mod_url/photo.php?lid=$newid&cid=$cid" ) ) ;
        }
    */
...
//$desc_tarea = new XoopsFormDhtmlTextArea( _BL_ALBM_PHOTODESC , "desc_text" , $myts->htmlSpecialChars( $photo['description'] ) , 10 , 50 ) ;
...
/*if ($myalbum_canrotate) {
    ...
    $rotate_radio->addOption( 'rot90' , "<img src='assets/images/icon_rotate90.gif' alt='"._BL_ALBM_RADIO_ROTATE90."' title='"._BL_ALBM_RADIO_ROTATE90."'> &nbsp; " ) ;
    $rotate_radio->addOption( 'rot180' , "<img src='assets/images/icon_rotate180.gif' alt='"._BL_ALBM_RADIO_ROTATE180."' title='"._BL_ALBM_RADIO_ROTATE180."'> &nbsp; " ) ;
    $rotate_radio->addOption( 'rot270' , "<img src='assets/images/icon_rotate270.gif' alt='"._BL_ALBM_RADIO_ROTATE270."' title='"._BL_ALBM_RADIO_ROTATE270."'> &nbsp; " ) ;
}*/
