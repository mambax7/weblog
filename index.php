...
require_once sprintf('%s/modules/%s/class/class.weblog.php', XOOPS_ROOT_PATH, $myModule->dirname());
require_once sprintf('%s/modules/%s/class/class.weblogcategories.php', XOOPS_ROOT_PATH, $myModule->dirname());
...
$date    = !empty($_GET['date']) ? (int)$_GET['date'] : 0;    // unixtime (archive.php is YYYYMM)

// privilege check
require_once sprintf('%s/modules/%s/include/privilege.inc.php', XOOPS_ROOT_PATH, $myModule->dirname());
$isAdmin = (isset($xoopsUser) && is_object($xoopsUser)) ? $xoopsUser->isAdmin($myModule->mid()) : false;
if (!$isAdmin && !checkprivilege('read_index', $myModule->dirname(), $cat_id)) {
    redirect_header(sprintf('%s/index.php', XOOPS_URL), 5, _BL_CANNOT_READ_SORRY);
}

// obtain configuration parameters
$perPage = $moduleHelper->getConfig('numperpage');
...
$isAdmin    = $currentUser->isAdmin($myModule->mid());
$currentuid = $currentUser->getVar('uid');

// specify template
$GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_entries.tpl';
...
if ($moduleHelper->getConfig('show_category_list')) {
    ...
    foreach ($cat_array as $cat) {
        ...
        if ($prefix_num <= 3) {
            $category['prefix'] = sprintf("<img src='%s/modules/%s/assets/images/cat%d%s.gif'>", XOOPS_URL, $moduleDirName, $prefix_num, $now);
        } else {
            for ($i = 3; $i < $prefix_num; ++$i) {
                $prefix_space .= sprintf("<img src='%s/modules/%s/assets/images/cat_space.gif'>", XOOPS_URL, $moduleDirName);
            }
            $category['prefix'] = $prefix_space . sprintf("<img src='%s/modules/%s/assets/images/cat%d%s.gif'>", XOOPS_URL, $moduleDirName, '3', $now);
        }
        $category_navi[] = $category;
    }
    $xoopsTpl->assign('category_col', $moduleHelper->getConfig('category_col'));
    $xoopsTpl->assign('category_navi', $category_navi);
}
$xoopsTpl->assign('show_category_list', $moduleHelper->getConfig('show_category_list'));
...
foreach ($weblog_result as $entryObject) {
    ...
    if ($updateReads === true) {
        ...
        setcookie(WEBLOG_COOKIE_READ, serialize($weblog_read), $curtime + $moduleHelper->getConfig('expiration'), $weblogCookie['path'], $weblogCookie['domain'], $weblogCookie['secure']);
        // wellwine for cookie ends
    }

    // create rdf
    require_once sprintf('%s/modules/%s/include/PEAR/Net/TrackBack.php', XOOPS_ROOT_PATH, $myModule->dirname());
    $weblog_trackback_url = XOOPS_URL . '/modules/' . $myModule->dirname() . '/weblog-tb.php/' . $entryObject->getVar('blog_id');
    ...
    $rdf_source_data = [
        ...
        'identifier'  => sprintf('%s/modules/%s/details.php?blog_id=%d', XOOPS_URL, $myModule->dirname(), $entryObject->getVar('blog_id')),
    ...
    $entry['created_date']  = formatTimestamp($entryObject->getVar('created'), $moduleHelper->getConfig('dateformat'), $xoopsConfig['default_TZ']);
    $entry['created_time']  = formatTimestamp($entryObject->getVar('created'), $moduleHelper->getConfig('timeformat'), $xoopsConfig['default_TZ']);
    ...
    $path                   = $weblogcat->getNicePathFromId($entryObject->getVar('cat_id'), sprintf('%s/modules/%s/index.php?user_id=%d', XOOPS_URL, $myModule->dirname(), $user_id));
    $entry['category']      = $path;
    ...
    $entry['provide_edit_link'] = ($isAdmin
                                   || ($currentuid == $entryObject->getVar('user_id')
                                       && checkprivilege('edit', $myModule->dirname()))) ? 1 : 0;
    $entry['lang_reads']        = _BL_NUMBER_OF_READS;
    $entry['reads']             = $entryObject->getVar('reads');
    $entry['trackbacks']        = $entryObject->getVar('trackbacks');
    $entry['tracklink']         = sprintf('%s/modules/%s/details.php?blog_id=%d#trackback', XOOPS_URL, $myModule->dirname(), $entryObject->getVar('blog_id'));
...
// wellwine
$rss = sprintf('%s/modules/%s/backend_weblog.php', XOOPS_URL, $myModule->dirname());
$rdf = sprintf('%s/modules/%s/weblog-rdf.php', XOOPS_URL, $myModule->dirname());
if ($user_id > 0) {
...
$xoopsTpl->assign('rss_show', $moduleHelper->getConfig('rssshow'));

$xoopsTpl->assign('page_title', $myModule->name());
$xoopsTpl->assign('page_subtitle', $page_subtitle);
$xoopsTpl->assign('xoops_weblogdir', $myModule->dirname());
...
$cat_url = sprintf('<a href=\'index.php?user_id=%d\'>%s</a>&nbsp;:&nbsp;', $user_id, _BL_MAIN);
$cat_url .= $weblogcat->getNicePathFromId($cat_id, sprintf('%s/modules/%s/index.php?user_id=%d', XOOPS_URL, $myModule->dirname(), $user_id));
$xoopsTpl->assign('cat_url', $cat_url);
//$xoopsTpl->assign('lang_blog', _BL_BLOG);
//$xoopsTpl->assign('lang_delete', _BL_DELETE);
$weblog_head = sprintf('<link rel="stylesheet" type="text/css" media="all" href="%s/modules/%s/styles.css">' . "\n", XOOPS_URL, $myModule->dirname());
