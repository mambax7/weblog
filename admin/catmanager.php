...
$mytree = new WeblogTree($xoopsDB->prefix($moduleDirName . '_category'), 'cat_id', 'cat_pid');
...
function catManagerLink()
{
    ...
    return sprintf('<a href=\'%s/modules/%s/admin/catmanager.php\'>%s</a>', XOOPS_URL, $xoopsModule->dirname(), _AM_WEBLOG_CATMANAGER);
}

function catManager()
{
    global $mytree, $xoopsModule, $xoopsModuleConfig, $adminObject, $currentFile;
...
//    require_once sprintf('%s/modules/%s/include/gtickets.php', XOOPS_ROOT_PATH, $xoopsModule->dirname());
    ...
    $moduleDirName = $GLOBALS['xoopsModule']->getVar('dirname');
    $gpermHandler  = xoops_getHandler('groupperm', $moduleDirName);
    ...
    $adminObject->addItemButton('Add', "{$currentFile}?op=downloads.list", 'list');
