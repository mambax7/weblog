...
if (!empty($_POST['post'])) {
    ...
    if ($goodPost) {
        ...
        if ($entry->isUpdateping()) {
            require_once sprintf('%s/modules/%s/class/class.commonping.php', XOOPS_ROOT_PATH, $xoopsModule->dirname());
            $weblog_url = sprintf('%s/modules/%s/', XOOPS_URL, $xoopsModule->dirname());
            if (!isset($blog_name)) {
                $blog_name = $xoopsModule->name() . ' - ' . sprintf(_BL_WHOS_BLOG, $xoopsUser->getVar('uname', 'E')) . ' - ' . $xoopsConfig['sitename'];
            }
            $Common_Updateping      = new Weblog_Commonping($blog_name, $weblog_url);
            $commonping_server_file = sprintf('%s/modules/%s/language/%s/commonping_servers.inc.php', XOOPS_ROOT_PATH, $xoopsModule->dirname(), $xoopsConfig['language']);
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_post.tpl';
    ...
    if ($xoopsModuleConfig['minentrysize'] > 0) {
        $blog_form->addElement(new XoopsFormLabel('', sprintf(_BL_POST_MUST_BE, $xoopsModuleConfig['minentrysize'])));
    }
    if ($xoopsModuleConfig['disable_html']) {
        $blog_form->addElement(new XoopsFormLabel(_BL_CAUTION_NOHTML, _BL_FORBIDDEN_HTML_TAGS));
    }
    $blog_form->addElement(new XoopsFormText(_BL_TITLE, 'title', $xoopsModuleConfig['editorwidth'], 80, $entry->getVar('title', 'e')), true);
    ...
    */
    ...
    $trackback_textarea = new XoopsFormTextArea(_BL_TRACKBACK_DSC . '<br>', 'ent_trackbackurl', $entry->getVar('ent_trackbackurl', 'e'), 4, $xoopsModuleConfig['editorwidth']);
    ...
    if (!empty($_POST['preview'])) {
        ...
        $xoopsTpl->assign('sample_date', formatTimestamp(time(), $xoopsModuleConfig['dateformat'], $xoopsConfig['default_TZ']));
    } else {
